/* Gloria Dal Santo
this is just another mod of the partconv patch
the main objective is that to load the impulse response from the header file 
*/
...
static void partconv_set(t_partconv *x, t_floatarg f)
{
	int i;
	int j;
	int arraypos;
	...
	post("partconv~: brir number %d, in %d partitions with FFT-size %d", x->brir_index, x->nparts, x->fftsize);
	x->ir_prepared = 1;
}
...
static void *partconv_new(t_symbol *s, int argc, t_atom *argv)
{
	...
	if (argc != 2) {
		post("argc = %d", argc);
		error("partconv~: usage: [partconv~ <brir_index> <partsize>]\n\t- partition size must be a power of 2 >= blocksize");
		return NULL;
	}

	x->brir_index = atom_getfloatarg(0, argc, argv);
...
void partconv_tilde_setup(void)
{
	...
	class_addmethod(partconv_class, (t_method) partconv_set, gensym("set"), A_DEFFLOAT, 0); // receives a number corresponding to the impulse response index in the header file 
}
